	...
	*/
	function httpQuery ($query) {
		...
		//if (ini_get('allow_url_fopen'))
			if($this->USE_PROXY==TRUE){
				$ch = stream_context_create(array('http'=>array('proxy'=>$this->PROXY . ':' . $this->PROXY_PORT)));
				$lines = @file($this->Query['server'], FALSE, $ch);
			}else{
	...
	function Connect ($server = '') {
		...
		while($retry <= $this->RETRY) {
			...
			if($this->USE_PROXY==TRUE){
				$ptr = @fsockopen($this->PROXY, $this->PROXY_PORT, $errno, $errstr, $this->STIMEOUT);
				fputs($ptr, "CONNECT $server:$port HTTP/1.0\r\n\r\n");
			}else{
